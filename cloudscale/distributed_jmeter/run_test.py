...
from cloudscale.distributed_jmeter.google import Google
...
def run_test(infrastructure, config_path, scenario_path, results_directory, logger):
    ...
    cfg = write_config(config_path, 'TEST')
    run_tests(cfg, userpath, r_path, scenario_path, infrastructure, logger)

    return userpath

def run_tests(cfg, user_path, r_path, scenario_path, infrastructure, logger):

    try:
        if infrastructure == 'aws':
            run_aws_test(r_path, user_path, scenario_path, cfg, logger)
        elif infrastructure == 'google':
            run_google_test(r_path, user_path, scenario_path, cfg, logger)
...
def write_config(config_path, section):
...
def run_google_test(r_path, output_path, scenario_path, cfg, logger):
    Google(cfg, scenario_path, r_path, output_path, logger)
