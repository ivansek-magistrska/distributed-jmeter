...
does_violates_1minute <- all(is.na(slo_df$when_violates))
if(!does_violates_1minute)
{
	common_1minute_gg <- common_1minute_gg + geom_text(data=slo_df, size=5, vjust=-1.5, aes(label=when_violates))
}
...
does_violates_5seconds <- all(is.na(slo_agg_5seconds_df$when_violates))
if(!does_violates_5seconds)
{
	common_5seconds_gg <- common_5seconds_gg + geom_text(data=slo_agg_5seconds_df, size=5, vjust=-1.5, aes(label=when_violates))
}
...
does_violates_10seconds <- all(is.na(slo_agg_10seconds_df$when_violates))
if(!does_violates_10seconds)
{
	common_10seconds_gg <- common_10seconds_gg + geom_text(data=slo_agg_10seconds_df, size=5, vjust=-1.5, aes(label=when_violates))
}
