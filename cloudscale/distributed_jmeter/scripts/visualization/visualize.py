...
class Visualize:
    ...
    def __init__(self, num_threads, duration, r_file, main_file, autoscaling_file):
        ...
        does_violates60, vus60, interval = data.slo_agg_seconds(self.parsed_file, self.jmeter_log_file, self.slo_violations_agg, 60)
        does_violates10, vus10, _ = data.slo_agg_seconds(self.parsed_file, self.jmeter_log_file, self.slo_violations_agg_10seconds, 10, skip=10)
        does_violates5, vus5, _ = data.slo_agg_seconds(self.parsed_file, self.jmeter_log_file, self.slo_violations_agg_5seconds, 5, skip=20)
        data.write_file_when_violates(
            num_threads,
            duration,
            vus60,
            does_violates60,
...
#    run_multiple(path)
     ...
     PATH = '/home/ivansek/projects/private/magistrska/gitlab.xlab.si/distributed-jmeter-standalone/bin/results/4891fa07-f87d-42db-aeb7-9b6d2e8001a4'
     ...
     R_FILE = '/home/ivansek/projects/private/magistrska/gitlab.xlab.si/distributed-jmeter-standalone/cloudscale/distributed_jmeter/scripts/visualization/r_visualization.R'
     import datetime
