...
class Parse:
    ...
    def slo_agg_seconds(self, parsed_file, jmeter_log_file, output_file, seconds, skip=0):
        ...
        with open(output_file, 'w') as slo_fp:
            with open(parsed_file) as fp:
                ...
                does_violates, vus_when_violates, interval_when_violates = self._write_file(slo_fp, timestamps, ref_timestamp, seconds, skip)
                return does_violates, vus_when_violates, interval_when_violates
    ...
    def _write_file(self, fp, timestamps, min_date, seconds, skip):
        ...
        fp.write(content)
        return violates, vus_when_violates, interval_when_violates
    ...
    def write_file_when_violates(self, num_threads, duration, vus60, does_violates60, interval_when_violates, vus10, does_violates10, vus5, does_violates5, ec2_file, rds_cpu_file, output_file):
        ...
        if not does_violates60:
            vus60 = num_threads
        with open(output_file, "w") as fp:
            fp.write("num_threads = %s\n" % num_threads)
            fp.write("duration = %s\n" % duration)
